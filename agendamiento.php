            ...
            <div class="p-5">
              ...
              <form class="user">
                <div><?php
                    ...
                    if($cupo_disponible>0 and !($rows>0) and strtotime($tiempo_fin)>strtotime($hora_actual) and $cedula!=""){
                        mail ( $correo , "Recordatorio cita" , "Recuerde que tiene una cita a en la franja ".
                        substr($row1["ft_inicio"],10)."-".$tiempo_fin."  
                        Una alarma sonará cuando falten 30 minutos para acabar el turno. ");
                    $admin="admin";
                    $conn=new mysqli("localhost","id14573840_jjpatinol","^yg6M1Ju>1Mm*sKS","id14573840_id1123581321_contrr");
                    $query= "INSERT INTO `detalleturno` (`id`, `cedula`, `id_horario`, `estado`, `admin`)
                    VALUES (NULL, '$cedula', '$id_horario', 'no atendido', 'admin')";
                    
                    $incrementar_cita="UPDATE `Horario` SET `num_personas` = `num_personas`+1  
                    ...
                    $conn->query($incrementar_cita);
                     echo '<h3 class="h4 text-dark-900 mb-4">Se ha registrado correctamente</h3>';
                        ...
                        echo '<h3 class="h4 text-dark-900 mb-4">Ya no existe cupo, ya se encuentra 
                        registrado en el horario escogido o el horario escogido ya venció</h3>';
                    }
                    ...
                    echo '<div class="text-center"><a class="small" href="userhome.php?usuario='.$cedula.'">Seguir registrando turnos</a></div>';
